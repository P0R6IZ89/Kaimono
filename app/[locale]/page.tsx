      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 pt-4">
        <Card className="p-0 justify-center outline-1 outline-dashed ring-0 bg-muted/50 border-transparent hover:outline-solid hover:outline-primary/50 hover:bg-accent/50 transition-colors">
          <Item>
            <ItemContent className="items-center text-center justify-center my-auto">
              <ItemTitle>{t("new-workspace-card.title")}</ItemTitle>
              <ItemDescription>
                {t("new-workspace-card.description")}
              </ItemDescription>
            </ItemContent>
          </Item>
          ...
          <CardContent className="pb-4">
            <Button asChild className="w-full" variant={"outline"} size={"sm"}>
              <span className="flex items-center">
                <Plus />
                <Link href="/new-workspace">{t("create-workspace")}</Link>
              </span>
            </Button>
          </CardContent>
        </Card>
        ...
        {apps.map((app) => {
          return (
            <Card
              key={app.id}
              className="h-full p-0 gap-3 shadow-md justify-between border-transparent hover:outline-solid hover:outline-primary/50 hover:bg-accent/50 transition-colors"
            >
              <Item className="">
                <ItemContent>
